...
$result=0;
$result=array_divide($data['post']);



function array_divide(array $input) {
    $result = reset($input);                            // first element of the array
    foreach (array_slice($input, 1) as $value) {        // use array_slice to avoid dividing the first element twice
